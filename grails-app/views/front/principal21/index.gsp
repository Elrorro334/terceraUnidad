    ...
    <div class="p-6 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center bg-gray-50 gap-4">
        ...
        <div class="flex flex-col sm:flex-row w-full md:w-auto space-y-3 sm:space-y-0 sm:space-x-3 items-center">
            <div class="flex w-full sm:w-80 relative">
                <input type="text" id="input-busqueda" placeholder="Buscar cliente o folio..." 
        ...
        <div class="p-6">
            <form id="form-estatico" onsubmit="guardarRegistro(event)">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="reg-cliente" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cobalto">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Monto de Operación ($) <span class="text-red-500">*</span></label>
                    <input type="number" id="reg-monto" required step="0.01" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cobalto">
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Estatus del CFDI <span class="text-red-500">*</span></label>
                    <select id="reg-estatus" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cobalto bg-white">
                ...
                <div class="flex justify-end space-x-3 border-t border-gray-200 pt-4">
                    ...
                    <button type="submit" class="px-4 py-2 bg-cobalto hover:bg-blue-800 text-white rounded-lg shadow-sm text-sm font-medium transition-colors">Timbrar (Simulado)</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function renderizarTabla() {
        ...
        if (terminoBusquedaActual) {
            datosFiltrados = datos.filter(function(d) {
                return d.cliente.toLowerCase().includes(terminoBusquedaActual) ||
        ...
        datosFiltrados.forEach(function(d) {
            const badgeClass = d.estatus === 'Vigente' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200';
            ...
            tbody.innerHTML += '<tr class="hover:bg-gray-50 transition-colors">' +
                ...
                '<td class="p-4"><span class="px-2 py-1 rounded-full text-xs font-semibold border ' + badgeClass + '">' + d.estatus + '</span></td>' +
    ...
    function guardarRegistro(e) {
        ...
        const cliente = sanitizarEntrada(document.getElementById('reg-cliente').value);
        ...
        if (id) {
            ...
            if (obj) {
                obj.cliente = cliente; 
                obj.monto = parseFloat(monto); 
                obj.estatus = estatus;
                if(typeof toastr !== 'undefined') toastr.success('Factura actualizada en memoria', 'Éxito');
            }
        } else {
            ...
            if(typeof toastr !== 'undefined') toastr.success('Factura timbrada (Modo simulación)', 'Éxito');
        }
        cerrarModal();
        renderizarTabla();
    }
